...
<title>[<% ident(); %>] <% translate("Advanced"); %>: <% translate("DHCP / DNS"); %></title>
...
<div class='section-title'><% translate("DHCP / DNS Server"); %> (LAN)</div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
	...
	{ title: '<a href="http://www.thekelleys.org.uk/" target="_new"><% translate("Dnsmasq"); %></a><br><% translate("Custom configuration"); %>', name: 'dnsmasq_custom', type: 'textarea', value: nvram.dnsmasq_custom }
]);
</script>

<!-- / / / -->

<div class='section-title'><% translate("DHCP Client"); %> (WAN)</div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
...
<li><b><% translate("Use received DNS with user-entered DNS"); %></b> - <% translate("Add DNS servers received from your WAN connection to the static DNS server list"); %> (<% translate("see"); %> <a href='basic-network.asp'><% translate("Network"); %></a> <% translate("configuration"); %>).</li>
...
<li><b><% translate("Ignore DHCP requests (...)"); %></b> - <% translate("Dnsmasq will ignore DHCP requests to Only MAC addresses listed on the"); %> <a href='basic-static.asp'><% translate("Static DHCP/ARP"); %></a> <% translate("page, others won't be able to obtain an IP address through DHCP"); %>.</li>
