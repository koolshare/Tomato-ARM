...
fog.dataToView = function(data) {
	return [(data[0] != '0') ? '<% translate("On"); %>' : '', ['<% translate("TCP"); %>', '<% translate("UDP"); %>', '<% translate("Both"); %>'][data[1] - 1], (data[2].match(/(.+)-(.+)/)) ? (RegExp.$1 + ' -<br>' + RegExp.$2) : data[2], data[3], data[4], data[5], data[6]];
}
...
fog.verifyFields = function(row, quiet) {
	...
	if (!v_nodelim(f[6], quiet, '<% translate("Description"); %>')) return 0;
	return 1;
}
