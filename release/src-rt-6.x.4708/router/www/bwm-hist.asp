...
var months = ['<% translate("Jan"); %>', '<% translate("Feb"); %>', '<% translate("Mar"); %>', '<% translate("Apr"); %>', '<% translate("May"); %>', '<% translate("Jun"); %>', '<% translate("Jul"); %>', '<% translate("Aug"); %>', '<% translate("Sep"); %>', '<% translate("Oct"); %>', '<% translate("Nov"); %>', '<% translate("Dec"); %>'];
var snames = [' KB', ' MB', ' GB'];
var scale = 2;
...
function checkRstats()
{
	if (nvram.rstats_enable != '1') {
		W('<div class="note-disabled"><% translate("Bandwidth monitoring disabled"); %>.</b><br><br><a href="admin-bwm.asp"><% translate("Enable"); %> &raquo;</a></div>');
		E('rstats').style.display = 'none';
	}
	else if (rstats_busy) {
		W('<div class="note-warning"><% translate("The rstats program is not responding or is busy. Try reloading after a few seconds"); %>.</div>');
	}
}
...
function checkCstats()
{
	if (nvram.cstats_enable != '1') {
		...
		W('<div class="note-warning"><% translate("The cstats program is not responding or is busy. Try reloading after a few seconds"); %>.</div>');
	}
}
