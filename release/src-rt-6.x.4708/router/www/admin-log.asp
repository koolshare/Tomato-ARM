...
createFieldTable('', [
	...
	{ title: '<% translate("Custom Log File Path"); %>', multi: [
		...
		{ name: 'log_file_path', type: 'text', maxlen: 50, size: 30, value: nvram.log_file_path, suffix: ' <small>(<% translate("make sure the directory exists and is writable"); %>)</small>' }
		] },
	...
	{ title: '<% translate("Connection Logging"); %>' },
		{ title: '<% translate("Inbound"); %>', indent: 2, name: 'log_in', type: 'select', options: [[0,'<% translate("Disabled (recommended)"); %>'],[1,'<% translate("If Blocked By Firewall"); %>'],[2,'<% translate("If Allowed By Firewall"); %>'],[3,'<% translate("Both"); %>']], value: nvram.log_in },
		{ title: '<% translate("Outbound"); %>', indent: 2, name: 'log_out', type: 'select', options: [[0,'<% translate("Disabled (recommended)"); %>'],[1,'<% translate("If Blocked By Firewall"); %>'],[2,'<% translate("If Allowed By Firewall"); %>'],[3,'<% translate("Both"); %>']], value: nvram.log_out },
...
createFieldTable('', [
	...
	{ title: '<% translate("Number of Entries to remember"); %>' },
		{ title: '<% translate("Domains"); %>', indent: 2,  name: 'log_wmdmax', type: 'text', maxlen: 4, size: 6, value: nvram.log_wmdmax, suffix: ' <small>(<% translate("0 to disable"); %>)</small>' },
		{ title: '<% translate("Searches"); %>', indent: 2, name: 'log_wmsmax', type: 'text', maxlen: 4, size: 6, value: nvram.log_wmsmax, suffix: ' <small>(<% translate("0 to disable"); %>)</small>' },
	{ title: '<% translate("Daily Backup"); %>', name: 'f_webmon_bkp', type: 'checkbox', value: nvram.webmon_bkp == 1, suffix: ' <small>(<% translate("every day at midnight"); %>)</small>' },
		{ title: '<% translate("Clear Data After Backup"); %>', indent: 2, name: 'f_webmon_shrink', type: 'checkbox', value: nvram.webmon_shrink == 1 },
		{ title: '<% translate("Backup Directory"); %>', indent: 2,  name: 'webmon_dir', type: 'text', maxlen: 128, size: 30, value: nvram.webmon_dir, suffix: ' <small>(<% translate("make sure the directory exists and is writable"); %>)</small>' }
]);
</script>
</div>
