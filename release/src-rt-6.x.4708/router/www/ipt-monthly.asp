...
<title>[<% ident(); %>] <% translate("IP Traffic"); %>: <% translate("Monthly History"); %></title>
	...
	if (monthly_history.length > 0) {
		...
		for (i = 0; i < monthly_history.length; ++i) {
			...
			if (E('_f_shortcuts').checked) {
				...
				h = h + '<a href="javascript:viewIptDetail(' + i + ')" title="<% translate("View real-time IP Traffic for this address"); %>">[<% translate("iptraf"); %>]</a>';
...
<div id='cstats'>
<div class='section-title'><% translate("IP Traffic Monthly History"); %></div>
<div class='section'>
...
&raquo; <a href="javascript:genData()"><% translate("Data"); %></a>
<br>
