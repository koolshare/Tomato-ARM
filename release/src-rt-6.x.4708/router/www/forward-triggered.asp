...
<title>[<% ident(); %>] <% translate("Forwarding"); %>: <% translate("Triggered"); %></title>
...
tg.dataToView = function(data) {
	return [data[0] ? '<% translate("On"); %>' : '', ['<% translate("TCP"); %>', '<% translate("UDP"); %>', '<% translate("Both"); %>'][data[1] - 1], data[2], data[3], data[4]];
}
...
tg.verifyFields = function(row, quiet) {
	...
	if (!v_nodelim(f[4], quiet, '<% translate("Description"); %>')) return 0;
	return 1;
}
...
<div class='section-title'><% translate("Triggered Port Forwarding"); %></div>
<div class='section'>
