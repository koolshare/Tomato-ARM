...
<title>[<% ident(); %>] VPN: <% translate("PPTP Server"); %></title>
...
ul.dataToView = function(data) {
	return [data[0], '<center><small><i><% translate("Secret"); %></i></small></center>'];
}
...
<div class='section-title'><% translate("PPTP Server Configuration"); %></div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
	...
	{ title: '<% translate("Broadcast Relay Mode"); %>', name: 'pptpd_broadcast', type: 'select', options: [['disable','<% translate("Disabled"); %>'], ['br0','<% translate("LAN to VPN Clients"); %>'], ['ppp','<% translate("VPN Clients to LAN"); %>'], ['br0ppp','<% translate("Both"); %>']], value: nvram.pptpd_broadcast },
	{ title: '<% translate("Encryption"); %>', name: 'pptpd_forcemppe', type: 'select', options: [[0, '<% translate("None"); %>'], [1, 'MPPE-128']], value: nvram.pptpd_forcemppe },
	...
	{ title: '<a href="http://poptop.sourceforge.net/" target="_new"><% translate("Poptop"); %></a><br><% translate("Custom configuration"); %>', name: 'pptpd_custom', type: 'textarea', value: nvram.pptpd_custom }

]);
</script>
</div>

<div class='section-title'><% translate("PPTP User List"); %></div>
<div class='section'>
...
<li><b><% translate("WINS Servers"); %></b> - <% translate("Allows configuring extra WINS servers for VPN clients, in addition to the WINS server defined on"); %> <a href=basic-network.asp><% translate("Basic"); %>/<% translate("Network"); %></a>.</li>
<li><b>MTU</b> - <% translate("Maximum Transmission Unit. Max packet size the PPTP interface will be able to send without packet fragmentation"); %>.</li>
<li><b>MRU</b> - <% translate("Maximum Receive Unit. Max packet size the PPTP interface will be able to receive without packet fragmentation"); %>.</li>
</ul>
...
&raquo; <a href="vpn-pptp-online.asp"><% translate("PPTP Online"); %></a>
</div>
