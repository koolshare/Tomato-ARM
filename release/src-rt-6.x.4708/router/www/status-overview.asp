...
createFieldTable('', [
	...
	{ title: '<% translate("Temperature"); %> <small>(CPU)</small>', rid: 'temps', text: stats.cputemp +'C'},
	...
	{ title: '<% translate("Wireless Temperature"); %>', rid: 'wlsense', text: stats.wlsense }
]);
</script>
</div>
...
for (var uidx = 1; uidx <= nvram.mwan_num; ++uidx) {
	var u = (uidx>1) ? uidx : '';
	W('<div class=\'section-title\' id=\'wan'+u+'-title\'>WAN'+u+' <small><i><a href=\'javascript:toggleVisibility("wan' + u + '");\'><span id=\'sesdiv_wan' +u + '_showhide\'>(<% translate("hide"); %>)</span></a></i></small></div>');
	W('<div class=\'section\' id=\'sesdiv_wan'+u+'\'>');
	createFieldTable('', [
		{ title: '<% translate("MAC Address"); %>', text: nvram['wan'+u+'_hwaddr'] },
		{ title: '<% translate("Connection Type"); %>', text: { 'dhcp':'<% translate("DHCP"); %>', 'static':'<% translate("Static IP"); %>', 'pppoe':'<% translate("PPPoE"); %>', 'pptp':'<% translate("PPTP"); %>', 'l2tp':'<% translate("L2TP"); %>', 'ppp3g':'<% translate("3G Modem"); %>', 'lte':'<% translate("4G/LTE"); %>' }[nvram['wan'+u+'_proto']] || '-' },
...
/* IPV6-END */
		...
		null,
		{ title: '<% translate("Status"); %>', rid: 'wan'+u+'status', text: stats.wanstatus[uidx-1] },
	...
	W('<input type=\'button\' class=\'controls\' onclick=\'dhcpc("renew","wan'+u+'")\' value=\'<% translate("Renew"); %>\'> &nbsp;');
	W('<input type=\'button\' class=\'controls\' onclick=\'dhcpc("release","wan'+u+'")\' value=\'<% translate("Release"); %>\'> &nbsp;');
	W('</span>');
	W('<input type=\'button\' class=\'controls\' onclick=\'wan_connect('+uidx+')\' value=\'<% translate("Connect"); %>\' id=\'b'+u+'_connect\' style=\'display:none\'>');
	W('<input type=\'button\' class=\'controls\' onclick=\'wan_disconnect('+uidx+')\' value=\'<% translate("Disconnect"); %>\' id=\'b'+u+'_disconnect\' style=\'display:none\'>');
	W('</div>');
}
</script>
