...
<title>[<% ident(); %>] <% translate("Bandwidth"); %>: <% translate("Last 24 Hours"); %></title>
...
{
	E('refresh-button').value = this.running ? '<% translate("Stop"); %>' : '<% translate("Start"); %>';
}
...
<div id='rstats'>
	...
	<div id='bwm-controls'>
		<small>(<% translate("2 minute interval"); %>)</small><br>
		<br>
		<% translate("Hours"); %>:&nbsp;
...
if (nvram.rstats_enable != '1') {
	...
	W('<div class="note-warning" style="display:none" id="rbusy"><% translate("The rstats program is not responding or is busy. Try reloading after a few seconds"); %>.</div>');
}
</script>
...
<tr><td id='footer' colspan=2>
	...
	<input type='button' value='<% translate("Refresh"); %>' id='refresh-button' onclick='ref.toggleX()'>
</td></tr>
</table>
</form>
</body>
