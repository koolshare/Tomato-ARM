...
/* CIFS-BEGIN */
		...
		if (nvram['cifs' + RegExp.$1].substr(0, 1) != '1') {
			ferror.set(eLoc, '<% translate("CIFS"); %> #' + RegExp.$1 + ' <% translate("is not enabled"); %>.', quiet || !ok);
			ok = 0;
		}
		else ferror.clear(eLoc);
	}
...
createFieldTable('', [
	...
	{ title: '<% translate("Database Location"); %>', multi: [
		{ name: 'f_loc', type: 'select', options: [['','RAM (<% translate("Temporary"); %>)'],
...
W('<br><input type="button" value="' + (mdup ? '<% translate("Restart"); %>' : '<% translate("Start"); %>') + ' <% translate("Now"); %>" onclick="restart(mdup)" id="_restart_button">');
</script>
</div>
