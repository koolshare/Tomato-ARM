...
<title>[<% ident(); %>] <% translate("Advanced"); %>: <% translate("Virtual Wireless Interfaces"); %></title>
...
<!-- / / / -->
<div class='section-title'><% translate("Virtual Wireless Interfaces"); %></div>
<div class='section'>
...
<div class='section-title'><% translate("Wireless Interfaces Details"); %> <small><i><a href='javascript:toggleVisibility("details");'><span id='sesdivdetailsshowhide'>(<% translate("Click here to show"); %>)</span></a></i></small></div>
...
<ul>
<!-- LINUX24-BEGIN -->
<li><b><% translate("Use alternate NAS startup"); %>(...)</b> - <i><% translate("Only meaningful for K24 builds"); %></i> - <% translate("Enable this option if you need more than one NAS process running"); %> (<% translate("i.e. to handle WPAx encryption on more than one WLVIF"); %>).</li>
<!-- LINUX24-END -->
</ul>
...
<ul>
<li><% translate("When creating/defining a new wireless VIF, it's MAC address will be shown (incorrectly) as '00:00:00:00:00:00', as it's unknown at that moment"); %> (<% translate("until network is restarted and this page is reloaded"); %>).</li>
<li><% translate("When saving changes, the MAC addresses of all defined non-primary wireless VIFs could sometimes be (already)"); %> <i><% translate("set"); %></i> <% translate("but might be"); %> <i><% translate("recreated"); %></i> <% translate("by the WL driver"); %> (<% translate("so that previously defined/saved settings might need to be updated/changed accordingly on"); %> <a href=advanced-mac.asp><% translate("Advanced"); %>/<% translate("MAC Address"); %></a> <% translate("after saving settings and rebooting your router"); %>).</li>
<li><% translate("This web interface allows configuring a maximum of 4 VIFs for each physical wireless interface available"); %> - <% translate("up to 3 extra VIFs can be defined in addition to the primary VIF"); %> (<i><% translate("on devices with multiple VIF capabilities"); %></i>).</li>
<li><% translate("By definition, configuration settings for the"); %> <i><% translate("primary VIF"); %></i> <% translate("of any physical wireless interfaces shouldn't be touched here"); %> (<% translate("use the"); %> <a href=basic-network.asp><% translate("Basic"); %>/<% translate("Network"); %></a>).</li>
</ul>
</ul>
</small>
...
for (var i = 1; i < tabs.length; ++i) {
	...
	W('<br>');
	W('VIF ' + tabs[i][1] + ' <% translate("is not defined"); %>.');
	W('</div>');
...
// only if primary VIF
	...
	f.push (
		...
		{ title: '<% translate("MAC Address"); %>', text: '<a href="advanced-mac.asp">' + (eval('nvram["wl'+u+'_hwaddr"]') || '00:00:00:00:00:00') + '</a>' +
			' &nbsp; <b id="wl'+u+'_hwaddr_msg" style="visibility:hidden"><small>(<% translate("warning: WL driver reports BSSID"); %> <a href=advanced-mac.asp>' + ((typeof(wl_ifaces[wl_ifidxx(u)]) != 'undefined')? wl_ifaces[wl_ifidxx(u)][9] : '') + '</a>)</small></b>' },
		{ title: '<% translate("Wireless Mode"); %>', name: 'f_wl'+u+'_mode', type: 'select',
			...
			suffix: ' &nbsp; <b id="wl'+u+'_mode_msg" style="visibility:hidden"><small>(<% translate("note: you might wish to cross-check settings later on"); %> <a href=basic-network.asp><% translate("Basic"); %>/<% translate("Network"); %></a>)</small></b>' }
	);
...
/* REMOVE-END */
	...
	for (var j = 1; j <= 4; ++j) {
		f.push(
			{ title: ('<% translate("Key"); %> ' + j), indent: 2, name: ('wl'+u+'_key' + j), type: 'text', maxlen: 26, size: 34,
...
<tr><td id='footer' colspan=2>
	...
	<input type='button' value='<% translate("Cancel"); %>' id='cancel-button' onclick='cancel()'>
</td></tr>
</table>
</form>
<script type='text/javascript'>
