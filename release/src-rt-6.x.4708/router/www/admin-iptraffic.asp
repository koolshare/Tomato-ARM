...
<title>[<% ident(); %>] <% translate("Admin"); %>: <% translate("IP Traffic Monitoring"); %></title>
...
function verifyFields(focused, quiet)
{
	...
	if (b) {
		...
		if (nvram.jffs2_on != '1') {
			ferror.set(eLoc, '<% translate("JFFS2 is not enabled"); %>.', quiet);
			return 0;
		}
	}
...
<div class='section-title'><% translate("IP Traffic Monitoring"); %></div>
...
REMOVE-END */
	...
	{ title: '<% translate("Create New File"); %><br><small>(<% translate("Reset Data"); %>)</small>', indent: 2, name: 'f_new', type: 'checkbox', value: 0,
		suffix: ' &nbsp; <b id="newmsg" style="visibility:hidden"><small>(<% translate("note: enable if this is a new file"); %>)</small></b>' },
	...
	{ title: '<% translate("Excluded IPs"); %>', name: 'cstats_exclude', type: 'text', value: nvram.cstats_exclude, maxlen: 512, size: 50, suffix: '&nbsp;<small>(<% translate("comma separated list"); %>)</small>' },
	{ title: '<% translate("Included IPs"); %>', name: 'cstats_include', type: 'text', value: nvram.cstats_include, maxlen: 2048, size: 50, suffix: '&nbsp;<small>(<% translate("comma separated list"); %>)</small>' },
	...
	{ title: '<% translate("Labels on graphics"); %>', name: 'cstats_labels', type: 'select', value: nvram.cstats_stime, options: [[0,'<% translate("Show known hostnames and IPs"); %>'],[1,'<% translate("Prefer to show only known hostnames, otherwise show IPs"); %>'],[2,'<% translate("Show only IPs"); %>']], value: nvram.cstats_labels }
]);
</script>
</form>
</div>
