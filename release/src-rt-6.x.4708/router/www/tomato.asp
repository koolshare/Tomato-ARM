...
TomatoRefresh.prototype = {
	...
	updateUI: function(mode) {
		...
		if ((e = E('refresh-button')) != null) {
			e.value = b ? '<% translate("Stop"); %>' : '<% translate("Refresh"); %>';
...
function navi()
{
	var menu = [
		...
		['<% translate("IP Traffic"); %>',			'ipt', 0, [
			...
			['<% translate("Transfer Rates"); %>',		'details.asp'],
			['<% translate("Daily"); %>',		'daily.asp'],
			['<% translate("Monthly"); %>',		'monthly.asp']
			]	],
		['<% translate("Tools"); %>', 				'tools', 0, [
			...
			['<% translate("System Commands"); %>',			'shell.asp'],
...
/* IPV6-END */
			...
			['<% translate("Static DHCP/ARP/IPT"); %>',		'static.asp'],
...
/* TOR-END */
			...
			['<% translate("Virtual Wireless"); %>',			'wlanvifs.asp'],
...
/* IPV6-END */
		...
		['<% translate("Bandwidth Limiter"); %>',		'bwlimit.asp'],
		null,	
/* NOCAT-BEGIN */
		['<% translate("Captive Portal"); %>',		'splashd.asp'],
/* NOCAT-END */
/* NGINX-BEGIN */
		['<% translate("Web Server"); %>',			'web', 0, [
			['<% translate("Nginx & PHP"); %>',		'nginx.asp'],
			['<% translate("MySQL Server"); %>',	'mysql.asp']
			] ],
/* NGINX-END */
/* REMOVE-BEGIN
		['Scripts',				'sc', 0, [
...
/* OPENVPN-BEGIN */
			['<% translate("OpenVPN Server"); %>',		'server.asp'],
			['<% translate("OpenVPN Client"); %>',		'client.asp'],
/* OPENVPN-END */
/* PPTPD-BEGIN */
			['<% translate("PPTP Server"); %>',			'pptp-server.asp'],
			['<% translate("PPTP Online"); %>',			'pptp-online.asp'],
			['<% translate("PPTP Client"); %>',			'pptp.asp']
/* PPTPD-END */
/* TINC-BEGIN */
			,['<% translate("Tinc Daemon"); %>',		'tinc.asp']
/* TINC-END */
		] ],
/* VPN-END */
		null,
		['<% translate("Administration"); %>',		'admin', 0, [
			...
			['<% translate("TomatoAnon"); %>',			'tomatoanon.asp'],
			['<% translate("Bandwidth Monitoring"); %>','bwm.asp'],
			['<% translate("IP Traffic Monitoring"); %>',	'iptraffic.asp'],
