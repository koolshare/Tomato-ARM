...
<title>[<% ident(); %>] <% translate("Advanced"); %>: <% translate("Conntrack / Netfilter"); %></title>
...
function check()
{
	...
	checker.onCompleted = function(text, xml) {
		...
		for (i = 1; i < 13; ++i) {
			E('count' + i).innerHTML = '&nbsp; <small>('+ ((conntrack[i] || 0) * 1) + ' <% translate("in this state"); %>)</small>';
		}
		E('count0').innerHTML = '(' + ((conntrack[0] || 0) * 1) + ' <% translate("connections currently tracked"); %>)';
		checker = null;
		timer.start(3000);
	}
...
function expireTimer()
{
	...
	if (--expireTime == 0) {
		...
		e.value = '<% translate("Expire Scheduled"); %>... ' + expireTime;
	}
}
...
<div class='section-title'><% translate("Connections"); %></div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
	{ title: '<% translate("Maximum Connections"); %>', name: 'ct_max', type: 'text', maxlen: 6, size: 8,
		suffix: '&nbsp; <a href="javascript:clicked()" id="count0">[ <% translate("count current"); %>... ]</a> <img src="spin.gif" style="vertical-align:bottom;padding-left:10px;visibility:hidden" id="spin" onclick="clicked()">',
		value: fixInt(nvram.ct_max || 4096, 128, 300000, 4096) }
/* LINUX26-BEGIN */
	,{ title: '<% translate("Hash Table Size"); %>', name: 'ct_hashsize', type: 'text', maxlen: 6, size: 8, value: nvram.ct_hashsize || 1023 }
/* LINUX26-END */
]);
</script>
<br>
<input type='button' value='<% translate("Drop Idle"); %>' onclick='expireClicked()' id='expire'>
<br><br>
</div>


<div class='section-title'><% translate("TCP Timeout"); %></div>
<div class='section'>
<script type='text/javascript'>
if ((v = nvram.ct_tcp_timeout.match(/^(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)$/)) == null) {
	v = [0,0,1200,120,60,120,120,10,60,30,0];
}
titles = ['-', '<% translate("None"); %>', '<% translate("Established"); %>', '<% translate("SYN Sent"); %>', '<% translate("SYN Received"); %>', '<% translate("FIN Wait"); %>', '<% translate("Time Wait"); %>', '<% translate("Close"); %>', '<% translate("Close Wait"); %>', '<% translate("Last ACK"); %>', '<% translate("Listen"); %>'];
...
<div class='section-title'><% translate("UDP Timeout"); %></div>
<div class='section'>
<script type='text/javascript'>
...
createFieldTable('', [
	{ title: ' ', text: '<small>(<% translate("seconds"); %>)</small>' },
	{ title: '<% translate("Unreplied"); %>', name: 'f_udp_0', type: 'text', maxlen: 6, size: 8, value: v[1], suffix: '<span id="count11"></span>' },
	{ title: '<% translate("Assured"); %>', name: 'f_udp_1', type: 'text', maxlen: 6, size: 8, value: v[2], suffix: '<span id="count12"></span>' }
]);
</script>
</div>

<div class='section-title'><% translate("Other Timeouts"); %></div>
<div class='section'>
<script type='text/javascript'>
...
createFieldTable('', [
	{ title: ' ', text: '<small>(<% translate("seconds"); %>)</small>' },
	{ title: '<% translate("Generic"); %>', name: 'f_ct_0', type: 'text', maxlen: 6, size: 8, value: v[1] },
	{ title: '<% translate("ICMP"); %>', name: 'f_ct_1', type: 'text', maxlen: 6, size: 8, value: v[2] }
]);
</script>
</div>

<div class='section-title'><% translate("Tracking / NAT Helpers"); %></div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
	{ title: '<% translate("FTP"); %>', name: 'f_ftp', type: 'checkbox', value: nvram.nf_ftp != '0' },
	{ title: '<% translate("GRE / PPTP"); %>', name: 'f_pptp', type: 'checkbox', value: nvram.nf_pptp != '0' },
	{ title: '<% translate("H.323"); %>', name: 'f_h323', type: 'checkbox', value: nvram.nf_h323 != '0' },
/* LINUX26-BEGIN */
	{ title: '<% translate("SIP"); %>', name: 'f_sip', type: 'checkbox', value: nvram.nf_sip != '0' },
/* LINUX26-END */
	{ title: '<% translate("RTSP"); %>', name: 'f_rtsp', type: 'checkbox', value: nvram.nf_rtsp != '0' }
]);
</script>
</div>
...
createFieldTable('', [
	{ title: '<% translate("TTL Adjust"); %>', multi: [
	...
	{ title: '<% translate("Inbound Layer 7"); %>', name: 'f_l7in', type: 'checkbox', value: nvram.nf_l7in != '0' }
]);
</script>
</div>
