...
for (var uidx = 1; uidx <= nvram.mwan_num; ++uidx) {
	...
	f.push(
		{ title: 'WAN'+u+' <% translate("Port"); %>', indent: 1, name: 'f_wan'+u+'_hwaddr', type: 'text', maxlen: 17, size: 20,
			suffix: ' <input type="button" value="<% translate("Default"); %>" onclick="bdefault(\'wan'+u+'\')"> <input type="button" value="<% translate("Random"); %>" onclick="brand(\'wan'+u+'\')"> <input type="button" value="<% translate("Clone PC"); %>" onclick="bclone(\'wan'+u+'\')">',
...
<table border=0 cellpadding=1>
	<tr><td><% translate("Router's LAN MAC Address"); %>:</td><td><b><script type='text/javascript'>W(('<% nv('et0macaddr'); %>').toUpperCase());</script></b></td></tr>
	<tr><td><% translate("Computer's MAC Address"); %>:</td><td><b><script type='text/javascript'>W(('<% compmac(); %>').toUpperCase());</script></b></td></tr>
</table>
</div>
