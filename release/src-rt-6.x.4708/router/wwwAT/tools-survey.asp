...
--><title><% translate("Wireless Survey"); %></title>
<content>
	<style type="text/css">
		#survey-grid .brate {
			color: blue;
		...
		sg.populate = function()
		{
			...
			for (i = 0; i < entries.length; ++i) {
				...
				if (useAjax()) {
					...
					if (m <= 10) seen += '<br> <small><% translate("NEW"); %> (' + -m + 'm)</small>';
				}

				mac = e.bssid;
				if (mac.match(/^(..):(..):(..)/))
					mac = '<a href="http://api.macvendors.com/' + RegExp.$1 + '-' + RegExp.$2 + '-' + RegExp.$3 + '" target="_new" title="<% translate("OUI search"); %>">' + mac + '</a>';
				sg.insert(-1, e, [
					'<small>' + seen + '</small>',
					'' + e.ssid,
					mac,
					(e.rssi == -999) ? '' : (e.rssi + ' <small><% translate("dBm"); %></small>'),
					(e.noise == -999) ? '' : (e.noise + ' <small><% translate("dBm"); %></small>'),
	...
	<div class="box">
		<div class="heading"><% translate("Wireless Survey"); %></div>
		<div class="content">
	...
	<div id="survey-controls">
		...
		<button type="button" value="<% translate("Refresh"); %>" onclick="ref.toggle();" id="refresh-button" class="btn"><% translate("Refresh"); %> <i class="icon-reboot"></i></button>
	</div>
