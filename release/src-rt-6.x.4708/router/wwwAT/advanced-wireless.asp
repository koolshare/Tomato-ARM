...
--><title><% translate("Wireless"); %></title>
<content>
	...
	<script type="text/javascript">
		...
		for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
			if (wl_sunit(uidx)<0) {
				...
				htmlOut += ('<div class="box"><div class="heading"><% translate("Wireless Settings"); %> ');
				...
				htmlOut += createFormFields([
					...
					{ title: '<% translate("802.11n Preamble"); %>', name: 'wl'+u+'_mimo_preamble', type: 'select', options: [['auto','<% translate("Auto"); %>'],['mm','<% translate("Mixed Mode"); %> *'],['gf','<% translate("Green Field"); %>'],['gfbcm','<% translate("GF-BRCM"); %>']],
					...
					{ title: '<% translate("Interference Mitigation"); %>', name: 'wl'+u+'_mitigation', type: 'select',
						options: [['0','<% translate("None"); %> *'],['1','<% translate("Non-WLAN"); %>'],['2','<% translate("WLAN Manual"); %>'],['3','<% translate("WLAN Auto"); %>'],['4','<% translate("WLAN Auto with Noise Reduction"); %>']],
						value: nvram['wl'+u+'_mitigation'] },
					{ title: '<% translate("WMM"); %>', name: 'wl'+u+'_wme', type: 'select', options: [['auto','<% translate("Auto"); %>'],['off','<% translate("Disable"); %>'],['on','<% translate("Enable"); %> *']], value: nvram['wl'+u+'_wme'] },
