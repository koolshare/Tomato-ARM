...
--><title><% translate("View Details"); %></title>
<content>
	<style type="text/css">
		#grid .co7 {
			width: 20px;
	...
	<script type="text/javascript">
				...
				for (j = cols.length-1; j >= 0; j--) {
					if (data[cols[j]] == ip) {
						...
						if (E('_f_shortcuts').checked)
							data[cols[j]] = data[cols[j]] + ' <small><a href="javascript:addExcludeList(\'' + ip + '\')" title="<% translate("Exclude from List"); %>">[<% translate("Hide"); %>]</a></small>';
		...
		ref.refresh = function(text) {
			...
			if (numconnshown != numconntotal)
				...
				E('numtotalconn').innerHTML='<small>(' + numconntotal + ' <% translate("connections"); %>)</small>';
		}
		...
		function init() {
			...
			if (viewClass != -1) $('#qos-details .heading').html('<% translate("QOS Details"); %>: ' + abc[viewClass] + ' <span id=\'numtotalconn\'></span>');
	...
	<div class="box" id="qos-details">
		<div class="heading"><% translate("QOS Details"); %> <span id="numtotalconn"></span></div>
		<div class="content">
			<h4><% translate("Filters"); %> <a href="javascript:toggleVisibility('filters');"><span id="sesdivfiltersshowhide"><i class="icon-chevron-up"></i></span></a></h4>
			...
			<div id="loading"><br><b><% translate("Loading"); %>...</b> <div class="spinner"></div></div>
		</div>
	</div>
