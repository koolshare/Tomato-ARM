...
<content>
	...
	<script type="text/javascript">
		...
		adblockg.setup = function() {
			...
			this.headerSet( [ '<% translate("On"); %>', 'URL', '<% translate("Description"); %>' ] );
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box">
			<div class="heading"><% translate("Adblock Settings"); %></div>
			<div class="content">
				...
				<h4><% translate("Blacklists"); %></h4>
				<table class="line-table" cellspacing=1 id="adblockg-grid"></table>
				<script type="text/javascript">adblockg.setup();</script>

				<br><hr>
				<h4><% translate("Custom List"); %></h4>
				<div class="blacklist_custom"></div><hr>
				<script type='text/javascript'>
					$( '.blacklist_custom' ).forms( [ { title: '<% translate("Blacklisted domains"); %>', name: 'adblock_blacklist_custom', type: 'textarea', value: nvram.adblock_blacklist_custom } ] );
				</script>

				<h4><% translate("Whitelist"); %></h4>
				<div class="whitelist"></div><hr>
				<script type="text/javascript">
					$( '.whitelist' ).forms( [ { title: '<% translate("Whitelisted domains"); %>', name: 'adblock_whitelist', type: 'textarea', value: nvram.adblock_whitelist } ] );
				</script>
		...
		<button type="button" value="<% translate("Cancel"); %>" id="cancel-button" onclick="javascript:reloadPage();" class="btn">Cancel <i class="icon-cancel"></i></button>
