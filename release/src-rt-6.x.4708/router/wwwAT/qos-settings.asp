...
<content>
	<script type="text/javascript">
		...
		function verifyFields(focused, quiet)
		{
			...
			if (!v_range('_wan_qos_obw', quiet, 10, 999999)) return 0;
			for (i = 0; i < 10; ++i)
			{
				if (!verifyClassCeilingAndRate(
					E('_wan_qos_obw').value,
					E('_f_orate_' + i).value,
					E('_f_oceil_' + i).value,
					'_okbps_' + i))
				{
			...
			if (!v_range('_wan_qos_ibw', quiet, 10, 999999)) return 0;
			for (i = 0; i < 10; ++i)
			{
				if (!verifyClassCeilingAndRate(
					E('_wan_qos_ibw').value,
					E('_f_irate_' + i).value,
					E('_f_iceil_' + i).value,
					'_ikbps_' + i))
				{
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="qos-out-limits">
			<div class="heading"><% translate("Outbound Rates / Limits"); %></div>
		...
		<div class="box" data-box="qos-class-n">
			<div class="heading"><% translate("QOS Class Names"); %></div>
