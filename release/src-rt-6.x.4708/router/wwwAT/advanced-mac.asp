...
--><title><% translate("MAC Address"); %></title>
<content>
	<style>
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box">
			<div class="heading"><% translate("Advanced MAC Settings"); %></div>
			<div class="content">
					...
					for (var uidx = 1; uidx <= nvram.mwan_num; ++uidx){
						...
						f.push(
							{ title: 'WAN'+u+' <% translate("Port"); %>', indent: 1, name: 'f_wan'+u+'_hwaddr', type: 'text', maxlen: 17, size: 20,
								suffix: ' <button type="button" value="<% translate("Default"); %>" onclick="bdefault(\'wan'+u+'\')" class="btn"><% translate("Default"); %></button> \
								<button type="button" value="<% translate("Random"); %>" onclick="brand(\'wan'+u+'\')" class="btn"><% translate("Random"); %></button> \
								<button type="button" value="<% translate("Clone PC"); %>" onclick="bclone(\'wan'+u+'\')" class="btn"><% translate("Clone PC"); %></button>',
