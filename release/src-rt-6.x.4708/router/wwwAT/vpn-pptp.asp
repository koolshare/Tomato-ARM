...
--><title><% translate("PPTP Client Configuration"); %></title>
<content>
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" id="pptp-client">
			<div class="heading"><% translate("PPTP Client Configuration"); %> <span class="pptp-client-status"></span></div>
			...
			<script type="text/javascript">
				$('#pptp-client .content').forms([
					...
					{ title: '<% translate("Accept DNS configuration"); %>', name: 'pptp_client_peerdns', type: 'select', options: [[0,'<% translate("Disabled"); %>'],[1,'<% translate("Yes"); %>'],[2,'<% translate("Exclusive"); %>']], value: nvram.pptp_client_peerdns },
					...
					{ title: '<% translate("Custom Configuration"); %>', name: 'pptp_client_custom', type: 'textarea', value: nvram.pptp_client_custom, style: "width: 100%; height: 80px;" }
				]);

				$('#pptp-client .pptp-client-status').html((!pptpup ? '<small style="color: red;">(<% translate("Stopped"); %>)</small>' : '<small style="color: green;">(<% translate("Running"); %>)</small>'));
				$('#pptp-client .pptp-client-status').after('<a href="#" data-toggle="tooltip" class="pull-right pptp-client-control" title="' +
					(pptpup ? '<% translate("Stop PPTP Client"); %>' : '<% translate("Start PPTP Client"); %>') + '" onclick="toggle(\'pptpclient\', pptpup); return false;" id="_pptpclient_button">' + (pptpup ? '<i class="icon-stop"></i>' : '<i class="icon-play"></i>') + '</a>');
