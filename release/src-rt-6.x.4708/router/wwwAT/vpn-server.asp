...
--><title><% translate("OpenVPN Server Configuration"); %></title>
<content>
	...
	<script type="text/javascript">
		...
		tabs = [['server1', '<% translate("VPN Server"); %> 1 <i class="icon-tools"></i>'],['server2', '<% translate("VPN Server"); %> 2 <i class="icon-tools"></i>']];
		...
		<div class="section" id="htmlConf">
			<script type="text/javascript">
				...
				for (i = 0; i < tabs.length; ++i)
				{
					...
					htmlOut += '<p class=\'keyhelp\'><% translate("For help generating keys, refer to the OpenVPN"); %> <a id=\''+t+'-keyhelp\'><% translate("HOWTO"); %></a>.</p>';
					htmlOut += createFormFields([
						...
						{ title: '<% translate("Server Certificate"); %>', name: 'vpn_'+t+'_crt', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_crt' ), style: 'width: 100%; height: 80px;' },
						{ title: '<% translate("Server Key"); %>', name: 'vpn_'+t+'_key', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_key' ), style: 'width: 100%; height: 80px;' },
						{ title: '<% translate("Diffie Hellman parameters"); %>', name: 'vpn_'+t+'_dh', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_dh' ), style: 'width: 100%; height: 80px;' }
					]);
					htmlOut += '</div>';
					htmlOut += '<div id=\''+t+'-status\'>';
					htmlOut += '<div id=\''+t+'-no-status\'><p><% translate("Server is not running or status could not be read"); %>.</p></div>';
					...
					htmlOut += '<div id=\''+t+'-status-clients\'><div class=\'section-title\'><% translate("Client List"); %></div><table class=\'line-table\' id=\''+t+'-status-clients-table\'></table><br></div>';
					htmlOut += '<div id=\''+t+'-status-routing\'><div class=\'section-title\'><% translate("Routing Table"); %></div><table class=\'line-table\' id=\''+t+'-status-routing-table\'></table><br></div>';
