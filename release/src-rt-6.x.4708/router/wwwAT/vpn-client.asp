...
--><title><% translate("OpenVPN Client Configuration"); %></title>
<content>
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<script type="text/javascript">
			...
			for (i = 0; i < tabs.length; ++i) {
				...
				var action = (eval('vpn'+(i+1)+'up') ? 'title="<% translate("Stop VPN Client"); %> ' + (i+1) + '"><i class="icon-stop"></i>' : 'title="<% translate("Start VPN Client"); %> ' + (i+1) + '"><i class="icon-play"></i>');
				var status = (!eval('vpn'+(i+1)+'up') ? '<small style="color: red">(<% translate("Stopped"); %>)</small>' : '<small style="color: green;">(<% translate("Running"); %>)</small>');

				htmlOut += '</ul>'
				+ '<div class="box"><div class="heading"><% translate("VPN Client"); %> #'+(i+1) + status + ' <a id="_vpn' + t + '_button" class="pull-right" href="#" data-toggle="tooltip"' +
				...
				htmlOut += createFormFields([
					...
					{ title: '<% translate("Accept DNS configuration"); %>', name: 'vpn_'+t+'_adns', type: 'select', options: [[0,'<% translate("Disabled"); %>'],[1, '<% translate("Relaxed"); %>'],[2, '<% translate("Strict"); %>'],[3,'<% translate("Exclusive"); %>']], value: eval( 'nvram.vpn_'+t+'_adns' ) },
					...
					{ title: '<% translate("Custom Configuration"); %>', name: 'vpn_'+t+'_custom', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_custom' ), style: 'width: 100%; height: 80px;' }
				]);
				htmlOut += ('</div>');
				...
				htmlOut += createFormFields([
					...
					{ title: '', suffix: '<table class=\'line-table\' id=\'table_'+t+'_routing\'></table>' }
				]);
				    htmlOut += ('<div>');
					htmlOut += ('<ul>');
				...
				htmlOut += ('<p class=\'keyhelp\'><% translate("For help generating keys, refer to the OpenVPN"); %> <a id=\''+t+'-keyhelp\'>HOWTO</a>.</p>');
				htmlOut += createFormFields([
					{ title: '<% translate("Static Key"); %>', name: 'vpn_'+t+'_static', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_static' ), style: 'width: 100%; height: 80px;' },
					{ title: '<% translate("Certificate Authority"); %>', name: 'vpn_'+t+'_ca', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_ca' ), style: 'width: 100%; height: 80px;' },
					{ title: '<% translate("Client Certificate"); %>', name: 'vpn_'+t+'_crt', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_crt' ), style: 'width: 100%; height: 80px;' },
					{ title: '<% translate("Client Key"); %>', name: 'vpn_'+t+'_key', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_key' ), style: 'width: 100%; height: 80px;' },
				]);
				htmlOut += ('</div>');

				htmlOut += ('<div id=\''+t+'-status\'>');
				htmlOut += ('<div id=\''+t+'-no-status\'><p><% translate("Client is not running or status could not be read"); %>.</p></div>');
				htmlOut += ('<div id=\''+t+'-status-content\' style=\'display:none\' class=\'status-content\'>');
				htmlOut += ('<div id=\''+t+'-status-header\' class=\'status-header\'><p><% translate("Data current as of"); %> <span id=\''+t+'-status-time\'></span>.</p></div>');
