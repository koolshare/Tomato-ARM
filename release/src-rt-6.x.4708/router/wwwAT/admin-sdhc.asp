<title><% translate("SDHC/MMC"); %></title>
<content>
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box">
			<div class="heading"><% translate("SDHC/MMC"); %></div>
			...
			<script type="text/javascript">
				...
				$('.content.form-sect').forms([
					{ title: '<% translate("Enable"); %>', name: 'f_mmc_on', type: 'checkbox', value: mmcon },
					...
					{ title: '<% translate("Total / Free Size"); %>', indent: 2, text: (scaleSize(mmc.size) + ' / ' + scaleSize(mmc.free) + ' <small>(' + (mmc.free/mmc.size*100).toFixed(2) + '%)</small>'), hidden: !mmc.size },
					null,
