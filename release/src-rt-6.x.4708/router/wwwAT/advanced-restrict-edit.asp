...
--><title><% translate("Edit Access Restrictions"); %></title>
<content>
	...
	<script type="text/javascript">
		...
		var ipp2p = [
			...
			[16,'<% translate("eDonkey"); %>'],[32,'<% translate("Gnutella"); %>'],[64,'<% translate("Kazaa"); %>'],[128,'<% translate("Mute"); %>'],[4096,'<% translate("PPLive/UUSee"); %>'],[256,'<% translate("SoulSeek"); %>'],[512,'<% translate("Waste"); %>'],[1024,'<% translate("WinMX"); %>']
			/* LINUX26-BEGIN */
			,[2048,'<% translate("XDCC"); %>'],[8192,'<% translate("Xunlei/QQCyclone"); %>']
			/* LINUX26-END */
		];
		...
		function save()
		{
			...
			if (data.length >= 8192) {
				alert('<% translate("This rule is too big. Please reduce by"); %> ' + (data.length - 8192) + ' <% translate("characters"); %>.');
				return;
			}
	...
	<form name="_fom" id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box">
			...
			<div class="content">
				...
				<button type="button" value="<% translate("Delete"); %>..." id="delete-button" onclick="delRULE();" class="btn btn-danger"><i class="icon-cancel"></i> <% translate("Delete"); %></button> &nbsp;
				...
				<button type="button" value="<% translate("Cancel"); %>" id="cancel-button" onclick="cancel();" class="btn"><% translate("Cancel"); %> <i class="icon-disable"></i></button>
