...
--><title><% translate("PPTP Server Configuration"); %></title>
<content>
	<script type="text/javascript">
		...
		ul.setup = function() {
			this.init('ul-grid', 'sort', 6, [
				...
				{ type: 'text', maxlen: 32, size: 32 } ]);
			this.headerSet(['<% translate("Username"); %>','<% translate("Password"); %>']);
		...
		function v_pptpd_secret(e, quiet) {
			...
			if (s.length < 1) {
				ferror.set(e, "<% translate("Username and password can not be empty"); %>.", quiet);
				return 0;
			}
			if (s.length > 32) {
				ferror.set(e, "<% translate("Invalid entry: max 32 characters are allowed"); %>.", quiet);
				return 0;
			}
			if (s.search(/^[.a-zA-Z0-9_\- ]+$/) == -1) {
				ferror.set(e, "<% translate("Invalid entry. Only characters"); %> \"A-Z 0-9 . - _\" <% translate("are allowed"); %>.", quiet);
				return 0;
			}
			e.value = s;
			ferror.clear(e);
		...
		ul.verifyFields = function(row, quiet) {
			...
			if (this.existUser(f[0].value)) {
				ferror.set(f[0], '<% translate("Duplicate User"); %>', quiet);
				return 0;
			}
		...
		ul.dataToView = function(data) {
			return [data[0], '<small><i><% translate("Secret"); %></i></small>'];
		}
		function save() {
			...
			if ((E('_f_pptpd_enable').checked) && (ul.getDataCount() < 1)) {
				var e = E('footer-msg');
				e.innerHTML = '<% translate("Cannot proceed: at least one user must be defined"); %>.';
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="pptp-server">
			<div class="heading"><% translate("PPTP Server"); %></div>
			<div class="content">

				<div id="pptp-server-settings"></div><hr>
				<script type="text/javascript">
					$('#pptp-server-settings').forms([
						{ title: '<% translate("Enable"); %>', name: 'f_pptpd_enable', type: 'checkbox', value: nvram.pptpd_enable == '1' },
						{ title: '<% translate("Local IP Address/Netmask"); %>', text: (nvram.lan_ipaddr + ' / ' + nvram.lan_netmask) },
						{ title: '<% translate("Remote IP Address Range"); %>', multi: [
						...
						{ title: '<% translate("Broadcast Relay Mode"); %>', name: 'pptpd_broadcast', type: 'select',
							options: [['disable','<% translate("Disabled"); %>'], ['br0','<% translate("LAN to VPN Clients"); %>'], ['ppp','<% translate("VPN Clients to LAN"); %>'], ['br0ppp','<% translate("Both"); %>']],
							value: nvram.pptpd_broadcast, suffix: ' <span style="color: red; font-size: 80%"><% translate("Enabling this may cause HIGH CPU usage"); %></span>' },
						{ title: '<% translate("Encryption"); %>', name: 'pptpd_forcemppe', type: 'select', options: [[0,'<% translate("None"); %>'], [1, '<% translate("MPPE-128"); %>']], value: nvram.pptpd_forcemppe },
						{ title: '<% translate("DNS Servers"); %>', name: 'pptpd_dns1', type: 'text', maxlen: 15, size: 17, value: nvram.pptpd_dns1 },
						{ title: '', name: 'pptpd_dns2', type: 'text', maxlen: 15, size: 17, value: nvram.pptpd_dns2 },
						{ title: '<% translate("WINS Servers"); %>', name: 'pptpd_wins1', type: 'text', maxlen: 15, size: 17, value: nvram.pptpd_wins1 },
						{ title: '', name: 'pptpd_wins2', type: 'text', maxlen: 15, size: 17, value: nvram.pptpd_wins2 },
						{ title: '<% translate("MTU"); %>', name: 'pptpd_mtu', type: 'text', maxlen: 4, size: 6, value: (nvram.pptpd_mtu ? nvram.pptpd_mtu : 1450)},
						{ title: '<% translate("MRU"); %>', name: 'pptpd_mru', type: 'text', maxlen: 4, size: 6, value: (nvram.pptpd_mru ? nvram.pptpd_mru : 1450)},
						{ title: '<a href="http://poptop.sourceforge.net/" target="_new">Poptop</a><br><% translate("Custom configuration"); %>', name: 'pptpd_custom', type: 'textarea', value: nvram.pptpd_custom, style: 'width: 100%; height: 80px;' }
					]);
				</script>
				...
				<div class="section" id="sesdiv_notes" style="display:none">
					<ul>
						<li><b><% translate("Local IP Address/Netmask"); %></b> - <% translate("Address to be used at the local end of the tunnelled PPP links between the server and the VPN clients"); %>.</li>
						<li><b><% translate("Remote IP Address Range"); %></b> - <% translate("Remote IP addresses to be used on the tunnelled PPP links (max 6)"); %>.</li>
						<li><b><% translate("Broadcast Relay Mode"); %></b> - <% translate("Turns on broadcast relay between VPN clients and LAN interface"); %>.</li>
						<li><b><% translate("Enable Encryption"); %></b> - <% translate("Enabling this option will turn on VPN channel encryption, but it might lead to reduced channel bandwidth"); %>.</li>
						<li><b><% translate("DNS Servers"); %></b> - <% translate("Allows defining DNS servers manually (if none are set, the router/local IP address should be used by VPN clients)"); %>.</li>
						<li><b><% translate("WINS Servers"); %></b> - <% translate("Allows configuring extra WINS servers for VPN clients, in addition to the WINS server defined on"); %> <a href=#basic-network.asp><% translate("Basic Network Settings"); %></a>.</li>
						<li><b><% translate("MTU"); %></b> - <% translate("Maximum Transmission Unit. Max packet size the PPTP interface will be able to send without packet fragmentation"); %>.</li>
						<li><b><% translate("MRU"); %></b> - <% translate("Maximum Receive Unit. Max packet size the PPTP interface will be able to receive without packet fragmentation"); %>.</li>
					</ul>
		...
		<div class="box" data-box="pptp-userlist">
			<div class="heading"><% translate("PPTP User List"); %> </div>
