...
TomatoGrid.prototype = {
	...
	rpMouIn: function(evt) {
		...
		s = '';
		if (me.canMove) {
			s = '<a class="move-up-row" href="#" onclick="this.parentNode.tgo.rpUp(this.parentNode.ref); return false;" title="<% translate("Move Up"); %>"><i class="icon-chevron-up"></i></a> \
			<a class="move-down-row" href="#" onclick="this.parentNode.tgo.rpDn(this.parentNode.ref); return false;" title="<% translate("Move Down"); %>"><i class="icon-chevron-down"></i></a> \
			<a class="move-row" href="#" onclick="this.parentNode.tgo.rpMo(this,this.parentNode.ref); return false;" title="<% translate("Move"); %>"><i class="icon-move"></i></a> ';
			n += 3;
		}
		if (me.canDelete) {
			s += '<a class="delete-row" href="#" onclick="this.parentNode.tgo.rpDel(this.parentNode.ref); return false;" title="<% translate("Delete"); %>"><i class="icon-cancel"></i></a>';
			++n;
		}
		x = PR(evt.target);
	...
	insert: function(at, data, cells, escCells) {
		...
		if ((this.canMove) || (this.canEdit) || (this.canDelete)) {
			...
			if (this.canEdit) e.title = '<% translate("Click to edit"); %>'; $(e).css('cursor', 'text');
		}
	...
	createControls: function(which, rowIndex) {
		...
		if (which == 'edit') {
			c.innerHTML = '<button type="button" class="btn btn-danger" value="<% translate("Delete"); %>" onclick="TGO(this).onDelete()"><% translate("Delete"); %> <i class="icon-cancel"></i></button> ' +
			'<button type="button" class="btn" value="<% translate("Cancel"); %>" onclick="TGO(this).onCancel()"><% translate("Cancel"); %> <i class="icon-disable"></i></button> ' +
			'<button type="button" class="btn btn-primary" value="OK" onclick="TGO(this).onOK()">OK <i class="icon-check"></i></button>';
		}
		else {
			c.innerHTML =
			'<button type="button" class="btn btn-danger" value="<% translate("Add"); %>" onclick="TGO(this).onAdd()"><% translate("Add"); %> <i class="icon-plus"></i></button>';
		}
		return r;
	},
...
TomatoRefresh.prototype = {
	...
	updateUI: function(mode) {
		...
		if ((e = E('refresh-button')) != null) {
			e.innerHTML = b ? '<% translate("Stop"); %>' : '<% translate("Refresh"); %>';
	...
	if (min >= 0) {
		...
		for (i = 0; i < t.length; ++i) {
			...
			if (v == 60) b.push('1 <% translate("minute"); %>');
				...
				else if (v == 1) b.push('1 <% translate("second"); %>');
...
function genStdRefresh(spin, min, exec)
{
	...
	html += genStdTimeList('refresh-time', '<% translate("Auto Refresh"); %>', min);
	html += '<button value="<% translate("Refresh"); %>" onclick="' + (exec ? exec : 'refreshClick()') + '; return false;" id="refresh-button" class="btn"><% translate("Refresh"); %> <i class="icon-reboot"></i></button></div>';
	return html;
}
...
function navi_icons ($name) {
	switch ($name) {
		case '<% translate("Status"); %>': 			return 'home'; break;
		case '<% translate("Basic Settings"); %>': 		return 'tools'; break;
		case '<% translate("Advanced Settings"); %>': 		return 'shield'; break;
		case '<% translate("Port Forwarding"); %>': 		return 'forward'; break;
		case '<% translate("Quality of Service"); %>': 		return 'gauge'; break;
		case '<% translate("USB & NAS"); %>': 			return 'drive'; break;
		case '<% translate("Web Services"); %>': 		return 'download'; break;
		case '<% translate("VPN"); %>': 			return 'globe'; break;
		case '<% translate("Administration"); %>': 		return 'wrench'; break;
...
function navi()
{
	...
	var menu = {
		'<% translate("Status"); %>': {
			'<% translate("Overview"); %>':			'status-home.asp',
			'<% translate("Device List"); %>':		'status-devices.asp',
			'<% translate("Web Usage"); %>':		'status-webmon.asp',
			'<% translate("Logs"); %>':			'status-log.asp'
		},
		'<% translate("Basic Settings"); %>': {
			'<% translate("Network"); %>':			'basic-network.asp',
			/* IPV6-BEGIN */
			'<% translate("IPv6"); %>':			'basic-ipv6.asp',
			/* IPV6-END */
			'<% translate("Identification"); %>':		'basic-ident.asp',
			'<% translate("Time"); %>':			'basic-time.asp',
			'<% translate("DDNS"); %>':			'basic-ddns.asp',
			'<% translate("DHCP/ARP/BW"); %>':		'basic-static.asp',
			'<% translate("Wireless Filter"); %>':		'basic-wfilter.asp'
		},
		'<% translate("Advanced Settings"); %>': {
			'<% translate("Access Restriction"); %>':	'advanced-restrict.asp',
			...
			/* TOR-END */
			'<% translate("Wireless"); %>':			'advanced-wireless.asp',
			'<% translate("VLAN"); %>':			'advanced-vlan.asp',
			'<% translate("LAN Access"); %>':		'advanced-access.asp',
			'<% translate("Virtual Wireless"); %>':		'advanced-wlanvifs.asp'
		},
		'<% translate("Port Forwarding"); %>': {
			'<% translate("Basic"); %>':			'forward-basic.asp',
			/* IPV6-BEGIN */
			'<% translate("Basic IPv6"); %>':		'forward-basic-ipv6.asp',
			/* IPV6-END */
			'<% translate("DMZ"); %>':			'forward-dmz.asp',
			'<% translate("Triggered"); %>':		'forward-triggered.asp',
			'<% translate("UPnP/NAT-PMP"); %>':		'forward-upnp.asp'
		},
		'<% translate("Quality of Service"); %>': {
			'<% translate("Basic Settings"); %>':		'qos-settings.asp',
			'<% translate("Classification"); %>':		'qos-classify.asp',
			'<% translate("View Graphs"); %>':		'qos-graphs.asp',
			'<% translate("View Details"); %>':		'qos-detailed.asp',
			'<% translate("Transfer Rates"); %>':		'qos-ctrate.asp',
			'<% translate("B/W Limiter"); %>':		'qos-qoslimit.asp'
		},
		/* USB-BEGIN */
		// ---- !!TB - USB, FTP, Samba, Media Server
		'<% translate("USB & NAS"); %>': {
			'<% translate("USB Support"); %>':		'nas-usb.asp'
			/* SAMBA-BEGIN */
			,'<% translate("File Sharing"); %>':		'nas-samba.asp'
			/* SAMBA-END */
			/* MEDIA-SRV-BEGIN */
			,'<% translate("Media Server"); %>':		'nas-media.asp'
			/* MEDIA-SRV-END */
			/* UPS-BEGIN */
			,'<% translate("UPS Monitor"); %>':		'nas-ups.asp'
			/* UPS-END */
			/* BT-BEGIN */
		...
		'<% translate("VPN Tunneling"); %>': {
			/* OPENVPN-BEGIN */
			'<% translate("OpenVPN Server"); %>':		'vpn-server.asp',
			'<% translate("OpenVPN Client"); %>':		'vpn-client.asp'
			/* OPENVPN-END */
			/* PPTPD-BEGIN */
			,'<% translate("PPTP Server"); %>':		'vpn-pptp-server.asp',
			'<% translate("PPTP Online"); %>':		'vpn-pptp-online.asp',
			'<% translate("PPTP Client"); %>':		'vpn-pptp.asp'
			/* PPTPD-END */
			/* TINC-BEGIN */
			,'<% translate("Tinc Daemon"); %>':		'vpn-tinc.asp'
			/* TINC-END */
		},
		/* VPN-END */
		'<% translate("Administration"); %>': {
			'<% translate("Admin Access"); %>':		'admin-access.asp',
			'<% translate("TomatoAnon"); %>':		'admin-tomatoanon.asp',
			'<% translate("Bandwidth Monitoring"); %>':	'admin-bwm.asp',
			'<% translate("IP Traffic Monitoring"); %>':	'admin-iptraffic.asp',
			'<% translate("Buttons/LED"); %>':		'admin-buttons.asp',
			/* CIFS-BEGIN */
			'<% translate("CIFS Client"); %>':		'admin-cifs.asp',
			/* CIFS-END */
			/* SDHC-BEGIN */
			'<% translate("SDHC/MMC"); %>':			'admin-sdhc.asp',
			/* SDHC-END */
			'<% translate("Configuration"); %>':		'admin-config.asp',
			'<% translate("Debugging"); %>':		'admin-debug.asp',
			/* JFFS2-BEGIN */
			'<% translate("JFFS"); %>':			'admin-jffs2.asp',
			/* JFFS2-END */
			/* NFS-BEGIN */
			'<% translate("NFS Server"); %>':		'admin-nfs.asp',
			/* NFS-END */
			'<% translate("Logging"); %>':			'admin-log.asp',
			'<% translate("Scheduler"); %>':		'admin-sched.asp',
			'<% translate("Scripts"); %>':			'admin-scripts.asp',
			/* SNMP-BEGIN */
			'<% translate("SNMP"); %>':			'admin-snmp.asp',
			/* SNMP-END */
			'<% translate("Upgrade"); %>':			'admin-upgrade.asp'
		}
	};

	// Add custom menu
	try { $.extend(true, menu, JSON && JSON.parse(nvram.web_nav) || $.parseJSON(nvram.web_nav)); } catch (e) { console.log('Failed to parse custom navigation (might not be set or inproperly formatted: use double quotes and no specialchars like tab or newline)'); }
