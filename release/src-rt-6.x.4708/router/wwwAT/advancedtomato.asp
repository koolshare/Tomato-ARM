	...
	if (typeof nvram.at_update !== "undefined" && nvram.at_update != '') {
		...
		if (n < lastUpdate || n == null) {
			...
			$updateNotification = $('<div class="alert alert-info icon"><a href="#" class="close" data-update="' + nvram.at_update.replace('.','') + '"><i class="icon-cancel"></i></a>\
				<h5><% translate("Update Available"); %>!</h5><% translate("AdvancedTomato version"); %> <b>' + nvram.at_update + '</b> <% translate("has been released and it is available for download"); %>.	&nbsp; <a target="_blank" href="https://advancedtomato.com/"><% translate("Click here to find out more"); %></a>.</div>');
...
function systemUI () {
	...
	$.ajax({
		...
		success: function( data ) {
			...
			content +=	'<div class="col-sm-2">CPU:</div><div class="col-sm-10">' + stats.cpuload + '</div>';
			content +=	'<div class="col-sm-2">T,&deg;C:</div><div class="col-sm-10">' + stats.cputemp + ' <small>CPU</small>';
			content +=	' / ' + stats.wl0temp + ' <small>WL0</small> / ' + stats.wl1temp + ' <small>WL1</small>';
			/* SMARTCTL-BEGIN */
			content +=	' / ' + stats.hddtemp + ' <small>HDD</small>';
			/* SMARTCTL-END */
			content +=	'</div><div class="col-sm-2">RAM:</div><div class="col-sm-10">' + stats.memory + '<div class="progress"><div class="bar" style="width: ' + stats.memoryperc + '"></div></div></div>';
			content +=	((nvram.swap != null) ? '<div class="col-sm-2">SWAP:</div><div class="col-sm-10">' + stats.swap + '<div class="progress"><div class="bar" style="width: ' + stats.swapperc + '"></div></div></div>':'');
			for(var uidx = 1; uidx <= nvram.mwan_num; ++uidx) {
				var u = (uidx>1) ? uidx : '';
				content +=	'<div class="col-sm-2">WAN'+u+':</div><div class="col-sm-10">' + stats.wanstatus[uidx-1] + ' (' + stats.wanuptime[uidx-1] + ')</div>';
			}
			content +=	'</div>';
			$('.system-ui .datasystem').html(content).removeClass('align center');
...
function loadPage( page, is_history ) {
	...
	$.ajax({
		...
		$('h2.currentpage').text( jqXHR.status + ' <% translate("ERROR"); %>');
		$('.container .ajaxwrap').html('<div class="box"><div class="heading"><% translate("ERROR"); %> - ' + jqXHR.status + '</div><div class="content">\
			<p><% translate("Unable to connect to the interface"); %>! <br><% translate("These issues usually occur when file is missing or router is unavailable to accept new connections"); %>.</p>\
			<a href="/"><% translate("Refreshing"); %></a> <% translate("browser window might help"); %>.</div></div>').addClass('ajax-animation');
