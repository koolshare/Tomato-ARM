...
--><title><% translate("BitTorrent Client"); %></title>
<content>
	<script type="text/javascript">
		...
		function verifyFields(focused, quiet)
		{
			...
			if (s.value.search(/"incomplete-dir":/) == 0)  {
				ferror.set(s, '<% translate("Cannot set"); %> "incomplete-dir" option here. If incomplete dir is enabled, all incomplete files will be downloaded to "/download_dir/.incomplete" directory.', quiet);
				ok = 0; }
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="bit-basic-set">
			<div class="heading"><% translate("BitTorrent Client"); %> - <% translate("Basic Settings"); %></div>
			<div class="content" id="torrent-config-section">

				<div class="clienttable"></div><hr>
				<script type="text/javascript">
					$('.clienttable').forms([
						{ title: '<% translate("Enable torrent client"); %>', name: 'f_bt_enable', type: 'checkbox', value: nvram.bt_enable == '1', suffix: ' *' },
						{ title: '<% translate("Transmission binary path"); %>', multi: [
							{ name: 'bt_binary', type: 'select', options: [
								...
								['custom','<% translate("Custom"); %>'] ], value: nvram.bt_binary, suffix: '*' },
		...
		<div class="box" data-box="bit-remote">
			<div class="heading"><% translate("Remote Access"); %> <span class="bt-link"></span></div>
			<div class="content">
				<div class="remoteacc"></div><hr>
				<script type="text/javascript">
					$('.remoteacc').forms([
						{ title: '<% translate("Enable GUI"); %>', name: 'f_bt_rpc_enable', type: 'checkbox', value: nvram.bt_rpc_enable == '1' },
						{ title: '<% translate("Listening GUI port"); %>', indent: 2, name: 'bt_port_gui', type: 'text', maxlen: 32, size: 5, value: nvram.bt_port_gui, suffix: '*' },
						{ title: '<% translate("Authentication required"); %>', name: 'f_bt_auth', type: 'checkbox', value: nvram.bt_auth == '1', suffix: ' *' },
						...
						{ title: '<% translate("Allow remote access"); %>', name: 'f_bt_rpc_wan', type: 'checkbox', value: nvram.bt_rpc_wan == '1', suffix: ' *' }
					]);
				</script>
				...
				<ul>
					...
					<li><b><% translate("Authentication required"); %></b> - <% translate("Authentication is <b><i>highly recomended</i></b>. GUI will prompt for user/pass"); %>.
		...
		<div class="box" data-box="bit-limits">
			<div class="heading"><% translate("Limits"); %></div>
			<div class="content">
		...
		<div class="box" data-box="bit-queue">
			<div class="heading"><% translate("Queue torrents"); %></div>
			<div class="content">
		...
		<div class="box" data-box="bit-advanced">
			...
			<script type="text/javascript">
				$('#tor-advancedsettings').forms([
					...
					null,
					{ title: '<a href="https://trac.transmissionbt.com/wiki/EditConfigFiles" target="_new">Transmission</a><br><% translate("Custom configuration"); %>', name: 'bt_custom', type: 'textarea', style: 'width: 100%; height: 63px;', value: nvram.bt_custom }
				]);
			</script>
		</div>
