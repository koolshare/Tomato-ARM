...
--><title><% translate("Trace Route"); %></title>
<content>
	<style type="text/css">
	...
	<script type="text/javascript">
		...
		tg.setup = function() {
			this.init('ttr-grid');
			this.headerSet(['<% translate("Hop"); %>', '<% translate("Address"); %>', '<% translate("Min"); %> (<% translate("ms"); %>)', '<% translate("Max"); %> (<% translate("ms"); %>)', '<% translate("Avg"); %> (<% translate("ms"); %>)', '<% translate("+/-"); %> (<% translate("ms"); %>)']);
		}
		...
		function verifyFields(focused, quiet)
		{
			...
			s = e.value.trim();
			if (!s.match(/^[\w\-\.\:]+$/)) {
				ferror.set(e, '<% translate("Invalid hostname/address"); %>', quiet);
				return 0;
			}
			ferror.clear(e);
		...
		function trace()
		{
			...
			tracer.onError = function(x) {
				spin(0);
				E('trace-error').innerHTML = '<% translate("ERROR"); %>: ' + E('_f_addr').value + ' - ' + x;
	...
	<ul class="nav-tabs">
		...
		<li><a class="active"><% translate("Trace"); %></a></li>
	...
	<div class="box">
		<div class="heading"><% translate("Trace Route"); %></div>
		<div class="content">
			...
			<script type="text/javascript">
				$('#tracert-form').forms([
					{ title: '<% translate("Address"); %>', name: 'f_addr', type: 'text', maxlen: 64, size: 32, value: '',
						suffix: ' <button type="submit" value="<% translate("Trace"); %>" onclick="trace()" id="traceb" class="btn btn-primary"><% translate("Trace"); %> <i class="icon-gauge"></i></button>' },
