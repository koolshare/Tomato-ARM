...
--><title><% translate("Wireless Filter"); %></title>
<content>
	...
	<script type="text/javascript">
		...
		smg.verifyFields = function(row, quiet) {
			...
			return v_mac(f[0], quiet) && v_nodelim(f[1], quiet, '<% translate("Description"); %>', 1);
		}
		...
		smg.setup = function() {
			...
			this.headerSet(['<% translate("MAC Address"); %>','<% translate("Description"); %>']);
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box">
			<div class="heading"><% translate("Wireless Filter"); %></div>
			<div class="content">
				<div class="radio c-radio"><label><input type="radio" name="f_type" id="_f_disable" value="disabled"><span class="icon-check"></span> <% translate("Disable filter"); %></label></div>
				&nbsp;
				<div class="radio c-radio"><label><input type="radio" name="f_type" id="_f_allow" value="allow"><span class="icon-check"></span> <% translate("Permit only the following clients"); %></label></div>
				&nbsp;
				<div class="radio c-radio"><label><input type="radio" name="f_type" id="_f_deny" value="deny"><span class="icon-check"></span> <% translate("Block the following clients"); %></label></div>
