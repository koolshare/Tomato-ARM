...
<content>
	...
	<script type="text/javascript">
		...
		pg.setup = function() {
			this.init('tp-grid');
			this.headerSet(['<% translate("Seq"); %>', '<% translate("Address"); %>', '<% translate("RX Bytes"); %>', 'TTL', 'RTT (<% translate("ms"); %>)', '+/- (<% translate("ms"); %>)']);
		}
		pg.populate = function()
		{
			...
			for (i = 0; i < buf.length; ++i) {
				if (r = buf[i].match(/^(\d+) bytes from (.+): .*seq=(\d+) ttl=(\d+) time=(\d+\.\d+) ms/)) {
					...
					stats = '<% translate("Round-Trip"); %>: ' + RegExp.$1 + ' <% translate("min"); %>, ' + RegExp.$2 + ' <% translate("avg"); %>, ' + RegExp.$3 + ' <% translate("max"); %> (<% translate("ms"); %>)<br>' + stats;
				}
			}
	...
	<ul class="nav-tabs">
		<li><a class="active"><% translate("Ping"); %></a></li>
			...
			<script type='text/javascript'>
				$('#ping-forms').forms([
					{ title: '<% translate("Address"); %>', name: 'f_addr', type: 'text', maxlen: 64, size: 32, value: '',
						suffix: ' <button type="submit" value="<% translate("Ping"); %>" onclick="ping()" id="pingb" class="btn btn-primary"><% translate("Ping"); %> <i class="icon-ping"></i></button>' },
