...
/* DUALWAN-END */
	...
	for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
		...
		if (i < 0) i = -i;
		stats.channel.push('<a class="ajaxload" href="#tools-survey.asp">' + ((i) ? i + '' : '<% translate("Auto"); %>') +
					((wlstats[uidx].mhz) ? ' - ' + (wlstats[uidx].mhz / 1000.0).toFixed(3) + ' <small><% translate("GHz"); %></small>' : '') + '</a>' +
					((a < 0) ? ' <small>(<% translate("scanning"); %>...)</small>' : ''));
		stats.interference.push((wlstats[uidx].intf >= 0) ? ((wlstats[uidx].intf) ? '<% translate("Severe"); %>' : '<% translate("Acceptable"); %>') : '');
		
		a = wlstats[uidx].nbw * 1;
		wlstats[uidx].nbw = (a > 0) ? (a + ' <small><% translate("MHz"); %></small>') : '<% translate("Auto"); %>';
		
		if (wlstats[uidx].radio) {
			a = wlstats[uidx].rate * 1;
			if (a > 0)
				wlstats[uidx].rate = Math.floor(a / 2) + ((a & 1) ? '.5' : '') + ' <small><% translate("Mbps"); %></small>';
			else
				wlstats[uidx].rate = '-';
			...
			wlstats[uidx].noise += ' <small><% translate("dBm"); %></small>';
			wlstats[uidx].rssi += ' <small><% translate("dBm"); %></small>';
		}
		else {
			wlstats[uidx].rate = '';
			wlstats[uidx].noise = '';
		...
		if (wl_ifaces[uidx][6] != 1) {
			wlstats[uidx].ifstatus = '<b><% translate("Down"); %></b>';
		} else {
