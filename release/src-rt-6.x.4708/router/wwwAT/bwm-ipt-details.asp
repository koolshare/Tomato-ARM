...
<content><style type="text/css">
	...
	<script type="text/javascript">
		...
		grid.setup = function() {
			this.init('grid', 'sort');
			this.headerSet(['<% translate("Host"); %>', '<% translate("Download (bytes/s)"); %>', '<% translate("Upload (bytes/s)"); %>', '<% translate("TCP IN/OUT (pkt/s)"); %>', '<% translate("UDP IN/OUT (pkt/s)"); %>', '<% translate("ICMP IN/OUT (pkt/s)"); %>', '<% translate("TCP Connections"); %>', '<% translate("UDP Connections"); %>']);
		}
	...
	<ul class="nav-tabs">
		...
		<li><a class="active"><i class="icon-globe"></i> <% translate("Transfer Rates"); %></a></li>
	...
	<div id="cstats" class="box">
		<div class="heading"><% translate("IP Traffic Details"); %></div>
		<div class="content">
