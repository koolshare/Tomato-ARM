...
<content>
	...
	<script type="text/javascript">
		...
		pbr.dataToView = function(data) {
			...
			if (data[2] != '0') {
				...
				if (data[3] != '') b.push(((data[2] == 1) ? '<% translate("IP"); %> ' : '<% translate("MAC"); %> ') + data[3]);
				else b.push('<% translate("All"); %>');
			...
			if (data[5] != '0') {
				if (data[6] != '') {
					switch (data[5]) {
						case '1':
							c.push('<% translate("IP"); %>: ' + data[6]);
							break;
						case '2':
							c.push('<% translate("MAC"); %>: ' + data[6]);
							break;
						case '3':
							c.push('<% translate("Domain"); %>: ' + data[6]);
							break;
					}
				}
			...
			if (data[1] >= -1) {
				if (data[1] == -1) a.push('<% translate("TCP/UDP"); %>');
					...
					else if (data[1] == 17) a.push('<% translate("UDP"); %>');
						else if (data[1] == 2) a.push('<% translate("ICMP"); %>');
				if (data[1] != 2) {
		...
		pbr.setup = function() {
			...
			var i, a, b;
			a = [[-2, '<% translate("All"); %>'],[-1,'<% translate("TCP/UDP"); %>'],[6,'<% translate("TCP"); %>'],[17,'<% translate("UDP"); %>'],[2, '<% translate("ICMP"); %>']];
			// what a mess...
			...
			this.headerSet(['<% translate("On"); %>', '<% translate("Protocol"); %>', '<% translate("Source Address"); %>', '<% translate("Destination Address"); %>', '<% translate("Select"); %> WAN', '<% translate("Description"); %>']);
    ...
    <form id="_fom" method="post" action="tomato.cgi">
        ...
        <div class="box">
            <div class="heading">M-WAN: <% translate("Routing Policy"); %></div>
            <div class="content">

                <table class="line-table" id="qg"></table>
                <br><b><% translate("Note"); %></b>: <% translate("Policy routing traffic only work on the LAN to the Internet"); %>.
