<title><% translate("Device List"); %></title>
<content>
	...
	<script type="text/javascript">
		...
		dg.populate = function()
		{
			...
			for (i = dhcpd_lease.length - 1; i >= 0; --i) {
				a = dhcpd_lease[i];
				e = get(a[2], a[1]);
				e.lease = '<a href="javascript:deleteLease(\'L' + i + '\',\'' + a[1] + '\')" title="<% translate("Delete Lease"); %>" id="L' + i + '">' + a[3] + ' <i class="icon-cancel icon-red"></i></a>';
				e.name = a[0];
				/* NOVLAN-BEGIN */
			...
			for (i = list.length - 1; i >= 0; --i) {
				...
				b = e.mac;
				if (e.mac.match(/^(..):(..):(..)/)) {
					b += '<br /><a class="btn btn-small" href="http://api.macvendors.com/' + RegExp.$1 + '-' + RegExp.$2 + '-' + RegExp.$3 + '" target="_new" title="<% translate("OUI Search"); %>">OUI</a> ' +
		...
		dg.setup = function()
		{
			...
			this.headerSet(['<% translate("Interface"); %>', '<% translate("MAC Address"); %>', '<% translate("IP (Hostname)"); %>', '<% translate("RSSI"); %>', '<% translate("Quality (%)"); %>', '<% translate("TX/RX Rate"); %>','<% translate("Lease"); %>']);
			this.populate();
			this.sort(2);
		}
	...
	<div class="box">
		<div class="heading"><% translate("Devices List"); %></div>
		<div class="content">
					...
					if (nvram['wl'+u+'_radio'] == '1') {
						if (wl_sunit(uidx)<0) {
							...
							if ((nvram['wl'+u+'_mode'] == 'ap') || (nvram['wl'+u+'_mode'] == 'wds'))
								a = '&nbsp; &nbsp; <button type="button" class="btn btn-primary" value="<% translate("Measure"); %>" onclick="javascript:window.location=\'wlmnoise.cgi?_http_id=' + nvram.http_id + '&_wl_unit=' + u +'\'">\
								<% translate("Measure"); %> <i class="icon-signal"></i></button>';
