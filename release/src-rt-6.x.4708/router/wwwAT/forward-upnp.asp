...
--><title><% translate("UPnP / NAT-PMP Forwarding"); %></title>
<content>
		...
		function verifyFields(focused, quiet)
		{
			...
			if ((enable) && (!E('_f_upnp_lan').checked) && (!E('_f_upnp_lan1').checked) && (!E('_f_upnp_lan2').checked) && (!E('_f_upnp_lan3').checked)) {
				if ((E('_f_enable_natpmp').checked) || (E('_f_enable_upnp').checked)) {
					var m = '<% translate("NAT-PMP or UPnP must be enabled in least one LAN bridge. You may continue but remember to configure an access to UPnP service by Custom Configuration"); %>';
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box">
			<div class="heading"><% translate("UPnP / NAT-PMP Port Forwarding"); %></div>
			<div class="content">
				<table id="upnp-grid" class="line-table"></table><br />
				<div style="width: 100%; text-align: right"><button type="button" value="<% translate("Delete All"); %>" onclick="deleteAll();" id="upnp-delete-all" class="btn btn-danger"><% translate("Delete All"); %> <i class="icon-cancel"></i></button>
					<button type="button" value="<% translate("Refresh"); %>" onclick="javascript:reloadPage();" class="btn"><i class="icon-reboot"></i> <% translate("Refresh"); %></button></div>
			</div>
		</div>

		<div class="box" data-box="forward-upnp-settings">
			<div class="heading"><% translate("Settings"); %></div>
